...
func ExecuteHydrate(ctx context.Context, params ExecParams) error {
	...
	validateOpts, err := hydrate.ValidateOptions(ctx, rootDir, params.APIServerTimeout)
	if err != nil {
		return err
	}
	...
	clusterFilterFunc := func(clusterName string, fileObjects []ast.FileObject, err status.MultiError) {
		clusterEnabled := allClusters(params.Clusters)
	...
	if params.Flat {
		err = hydrate.PrintFlatOutput(params.OutPath, params.OutputFormat, fileObjects)
	} else {
		err = hydrate.PrintDirectoryOutput(params.OutPath, params.OutputFormat, fileObjects)
	}
	if err != nil {
		return err
	}
