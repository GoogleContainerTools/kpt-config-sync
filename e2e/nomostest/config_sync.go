...
func SetupFakeSSHCreds(nt *NT, rsKind string, rsRef types.NamespacedName, auth configsync.AuthType, secretName string) error {
	...
	if rsKind == kinds.RootSyncV1Beta1().Kind {
		if err := nt.Watcher.WatchForRootSyncStalledError(rsRef.Name, "Validation", validate.MissingSecret(secretName).Error(), nil); err != nil {
			return err
		}
	} else {
		if err := nt.Watcher.WatchForRepoSyncStalledError(rsRef.Namespace, rsRef.Name, "Validation", validate.MissingSecret(secretName).Error(), nil); err != nil {
			return err
		}
	}
