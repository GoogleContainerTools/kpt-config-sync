...
func notificationWebhookPodName(nt *NT) (string, error) {
	...
	err := nt.KubeClient.List(podList, client.InNamespace(testNotificationWebhookNamespace))
	if err != nil {
		return "", err
	}
...
// NotificationConfigMapRef is the name of the notification test ConfigMap
const NotificationConfigMapRef = "test-notification-cm"

// NotificationSecretRef is the name of the notification test Secret
...
func SubscribeRootSyncNotification(rootSync *v1beta1.RootSync, trigger, service string) {
	...
	subscribeAnnotation := SubscribeAnnotationKey(trigger, service)
...
func NotificationConfigMap(nt *NT, ns string, mutators ...ConfigMapMutator) (*corev1.ConfigMap, error) {
	...
	err := nt.KubeClient.Create(cm)
	nt.T.Cleanup(func() {
		if err := nt.KubeClient.Delete(cm); err != nil && !apierrors.IsNotFound(err) {
			nt.T.Error(err)
		}
	})
	return cm, err
...
func NotificationSecret(nt *NT, ns string, mutators ...SecretMutator) (*corev1.Secret, error) {
	...
	err := nt.KubeClient.Create(secret)
	nt.T.Cleanup(func() {
