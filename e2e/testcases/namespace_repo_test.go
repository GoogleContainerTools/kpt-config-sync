...
func validateRepoSyncRBAC(nt *nomostest.NT, ns string, nsRepo *gitproviders.ReadWriteRepository, configureRBAC configureRBACFunc) {
	...
	nt.Must(nt.WaitForRepoSyncSyncError(ns, configsync.RepoSyncName, applier.ApplierErrorCode,
...
func TestManageSelfRepoSync(t *testing.T) {
	...
	nt.Must(nt.WaitForRepoSyncSourceError(rs.Namespace, rs.Name, validate.SelfReconcileErrorCode, "RepoSync bookstore/repo-sync must not manage itself in its repo"))
}
