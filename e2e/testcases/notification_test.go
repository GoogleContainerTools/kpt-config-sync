...
func TestNotification(t *testing.T) {
	...
	nt.Must(nt.RootRepos[rootSyncNN.Name].Add(nomostest.StructuredNSPath(repoSyncNN.Namespace, repoSyncNN.Name), repoSyncBackend))
	nt.Must(nt.RootRepos[rootSyncNN.Name].CommitAndPush("Enable notifications on backend RepoSync"))
	tg = taskgroup.New()
	tg.Go(func() error {
	...
	nt.Must(nt.RootRepos[rootSyncNN.Name].Add(nomostest.StructuredNSPath(repoSyncNN.Namespace, repoSyncNN.Name), repoSyncBackend))
	nt.Must(nt.RootRepos[rootSyncNN.Name].CommitAndPush("Disable notifications on backend RepoSync"))
