...
func TestSyncingThroughAProxy(t *testing.T) {
	...
	nt.Must(nt.Watcher.WatchForRootSyncStalledError(rs.Name, "Validation",
		validate.MissingKeyInSecret(`git secretType was set as "cookiefile" but cookie_file key is not present in git-creds secret`).Error()))
	nt.T.Log("Set auth type to token")
	nt.MustMergePatch(rs, `{"spec": {"git": {"auth": "token"}}}`)
	nt.T.Log("Verify the secretRef error")
	nt.Must(nt.Watcher.WatchForRootSyncStalledError(rs.Name, "Validation",
		validate.MissingKeyInSecret(`git secretType was set as "token" but token key is not present in git-creds secret`).Error()))
