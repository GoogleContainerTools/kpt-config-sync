...
func TestManagingReconciler(t *testing.T) {
	...
	err = nt.Watcher.WatchObject(kinds.Deployment(), nomostest.DefaultRootReconcilerName, v1.NSConfigManagementSystem,
		[]testpredicates.Predicate{testpredicates.HasGenerationAtLeast(generation), firstContainerTerminationMessagePathEquals("dev/termination-message"),
			firstContainerStdinEquals(true), hasTolerations(modifiedTolerations), hasPriorityClassName("system-node-critical")})
	if err != nil {
		nt.T.Fatal(err)
	}
	generation = getDeploymentGeneration(nt, nomostest.DefaultRootReconcilerName, v1.NSConfigManagementSystem)
	...
	err = nt.Watcher.WatchObject(kinds.Deployment(), nomostest.DefaultRootReconcilerName, v1.NSConfigManagementSystem,
		[]testpredicates.Predicate{testpredicates.HasGenerationAtLeast(generation), firstContainerTerminationMessagePathEquals("dev/termination-log"),
			firstContainerStdinEquals(false), hasTolerations(originalTolerations), hasPriorityClassName("")})
	if err != nil {
		nt.T.Fatal(err)
	}
	generation = getDeploymentGeneration(nt, nomostest.DefaultRootReconcilerName, v1.NSConfigManagementSystem)
...
func TestAutopilotReconcilerAdjustment(t *testing.T) {
	nt := nomostest.New(t, nomostesting.ACMController, ntopts.Unstructured,
		// Disable Autoscaling so we can validate the Autopilot & Standard behavior.
		// Autoscaling behavior is inconsistent enough that we can't really
		// predict exactly what the resulting resource values will be.
	...
	err := nt.Validate(rootSyncNN.Name, rootSyncNN.Namespace, rootSyncObj,
		// Confirm autoscaling is disabled
		testpredicates.MissingReconcilerAutoscalingStrategy(),
	...
	// Default values depend on whether VPA is being used or not.
	...
	if *e2e.VPA && nomostest.IsReconcilerAutoscalingEnabled(rootSyncObj) {
		expectedResources = controllers.ReconcilerContainerResourceAutoscaleDefaults()
	} else {
	...
	err = nt.Validate(reconcilerNN.Name, reconcilerNN.Namespace, reconcilerDeployment,
		...
		testpredicates.DeploymentContainerResourcesAllEqual(expectedResources),
	)
	if err != nil {
