...
func TestForceRevert(t *testing.T) {
	...
	nt.Must(nt.WaitForRootSyncSourceError(configsync.RootSyncName, system.MissingRepoErrorCode, ""))
...
func TestManageSelfRootSync(t *testing.T) {
	...
	nt.Must(nt.WaitForRootSyncSourceError(configsync.RootSyncName, validate.SelfReconcileErrorCode, "RootSync config-management-system/root-sync must not manage itself in its repo"))
}
