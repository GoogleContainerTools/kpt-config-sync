...
// UnstructuredInventoryConverter is an interface for converting ResourceGroup
// objects to/from UnstructuredInventory objects.
// This provides the translation logic to instantiate a UnstructuredClient which
// works with ResourceGroup objects.
type UnstructuredInventoryConverter interface {
	// InventoryFromUnstructured converts a ResourceGroup object to UnstructuredInventory
	InventoryFromUnstructured(fromObj *unstructured.Unstructured) (*inventory.UnstructuredInventory, error)
	...
	InventoryToUnstructured(fromObj *unstructured.Unstructured, toInv *inventory.UnstructuredInventory) (*unstructured.Unstructured, error)
}
...
func (ic *unstructuredInventoryConverter) InventoryToUnstructured(fromObj *unstructured.Unstructured, toInv *inventory.UnstructuredInventory) (*unstructured.Unstructured, error) {
	...
	for _, s := range toInv.ObjectStatuses() {
	...
	klog.V(4).Infof("creating list of %d resources", len(toInv.ObjectRefs()))
	var objs []interface{}
	for _, objMeta := range toInv.ObjectRefs() {
	...
	klog.V(4).Infof("Creating list of %d resource statuses", len(toInv.ObjectRefs()))
	...
	if len(objs) == 0 { // Unset resource fields
		...
		unstructured.RemoveNestedField(toObj.UnstructuredContent(),
			"spec", "resources")
		unstructured.RemoveNestedField(toObj.UnstructuredContent(),
			"status", "resourceStatuses")
	} else { // Update resource fields
		klog.V(4).Infof("storing inventory (%d) resources", len(objs))
		err := unstructured.SetNestedSlice(toObj.UnstructuredContent(),
			objs, "spec", "resources")
		if err != nil {
			return nil, err
		}
	...
	err := unstructured.SetNestedField(toObj.UnstructuredContent(),
