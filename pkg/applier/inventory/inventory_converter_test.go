...
func (f resourceGroupFactory) build(t *testing.T) *unstructured.Unstructured {
	...
	rg.SetGeneration(int64(invGeneration))
	rg.Spec = f.spec
	rg.Status = f.status
...
func TestInventoryFromUnstructured(t *testing.T) {
	testCases := map[string]struct {
		...
		"ResourceGroup with spec and status": {
			fromObj: resourceGroupFactory{
				...
				status: v1alpha1.ResourceGroupStatus{
					ObservedGeneration: int64(invGeneration),
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			require.Equal(t, tc.wantObjRefs, inv.ObjectRefs())
			require.Equal(t, tc.wantObjStatuses, inv.ObjectStatuses())
		})
	}
}
...
func TestInventoryToUnstructured(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			if !equality.Semantic.DeepEqual(rg, tc.wantObj) {
				t.Fatalf("ResourceGroup does not match expected:\nDiff (- Expected, + Found)\n%s",
					log.AsYAMLDiff(tc.wantObj, rg))
			}
		})
	}
}
...
func TestNewInventoryConverter(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			ici := NewInventoryConverter(tc.scope, invName, metadata.StatusEnabled)
			ic := ici.(*unstructuredInventoryConverter)
