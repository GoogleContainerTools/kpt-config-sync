...
func TestNew(t *testing.T) {
	...
	testCases := []struct {
		...
		expectedErrCount int // TODO: Check for exact errors instead?
		contextErr       error
	...
	}{
		{
			...
			client:           fake.NewClientBuilder().WithScheme(core.Scheme).WithObjects(csNS).Build(),
		...
		{
			name:             "multi-repo",
			client:           fake.NewClientBuilder().WithObjects(csNS, cmMultiRepo).Build(),
			clientset:        k8sfake.NewSimpleClientset(),
			expectedErrCount: 0,
			expectedOutput:   "",
			checkFn: func(t *testing.T, br *BugReporter) {
		...
		{
			name:             "mono-repo",
			client:           fake.NewClientBuilder().WithObjects(csNS, cmMonoRepo).Build(),
			clientset:        k8sfake.NewSimpleClientset(),
			expectedErrCount: 0,
