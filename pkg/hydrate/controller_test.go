...
func TestBlockHydaration(t *testing.T) {
	commit := "abcd123"
	anotherCommit := "abcd12345"
	testCases := []struct {
		name                    string
		isReadyToRender         bool
		readyToRenderFileExists bool
		sourceCommit            string
		readyToRenderCommit     string
	}{
		{
			name:            "ready-to-render file does not exist",
			isReadyToRender: false,
		},
		{
			name:                    "ready-to-render file contains commit different from sourceCommit",
			isReadyToRender:         false,
			readyToRenderFileExists: true,
			readyToRenderCommit:     anotherCommit,
		},
		{
			name:                    "ready-to-render file has empty content",
			isReadyToRender:         false,
			readyToRenderFileExists: true,
			readyToRenderCommit:     "",
		},
		{
			name:                    "ready-to-render file contains commit same as sourceCommit",
			isReadyToRender:         true,
			readyToRenderFileExists: true,
			readyToRenderCommit:     commit,
		},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			// create test related directories and files as configured
			srcRootDir := filepath.Join(os.TempDir(), "test-srcCommit-syncDir", rand.String(10))
			reconcilerSignalsDir := filepath.Join(srcRootDir, "reconciler-signals")
			commitDir := filepath.Join(srcRootDir, commit)
			if err := os.MkdirAll(srcRootDir, 0700); err != nil {
				t.Errorf("failed to create source root directory %q: %v", srcRootDir, err)
			}
			if tc.readyToRenderFileExists {
				if err := os.MkdirAll(reconcilerSignalsDir, 0700); err != nil {
					t.Errorf("failed to create reconciler-signals directory %q: %v", srcRootDir, err)
				}
				ReadyToRenderFilePath := filepath.Join(reconcilerSignalsDir, ReadyToRenderFile)
				if err := os.WriteFile(ReadyToRenderFilePath, []byte(tc.readyToRenderCommit), 0644); err != nil {
					t.Errorf("failed to write to ready-to-render file %q: %v", ReadyToRenderFilePath, err)
				}
			}
			...
			hydrator := &Hydrator{
				...
				RehydratePeriod:     1 * time.Minute,
				ReconcilerName:      "root-reconciler",
				ReconcilerSignalDir: cmpath.Absolute(reconcilerSignalsDir),
			}

			t.Logf("start calling isReadyToRender at %v", time.Now())
			readyToRenderCommit, isReadyToRender := hydrator.isReadyToRender(commit)
			assert.Equal(t, tc.readyToRenderCommit, readyToRenderCommit)
			assert.Equal(t, tc.isReadyToRender, isReadyToRender)
		})
	}
}
