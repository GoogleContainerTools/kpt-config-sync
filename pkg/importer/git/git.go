...
// SyncError returns the error details from the error file generated by the git-sync process.
func SyncError(filepath, label string) string {
	content, err := ioutil.ReadFile(filepath)
	if err != nil {
		return fmt.Sprintf("Unable to load %s: %v. Please check git-sync logs for more info: kubectl logs -n config-management-system -l %s -c git-sync", filepath, err, label)
	}
	if len(content) == 0 {
		return fmt.Sprintf("%s is empty. Please check git-sync logs for more info: kubectl logs -n config-management-system -l %s -c git-sync", filepath, label)
	}
	return fmt.Sprintf("Error in the git-sync container: %s", string(content))
}
