...
func ToTypedWithVersion(obj runtime.Object, targetGVK schema.GroupVersionKind, scheme *runtime.Scheme) (runtime.Object, error) {
	...
	if gvk != targetGVK {
		// Convert to internal version (typed)
		iObj, err := scheme.New(targetGVK.GroupKind().WithVersion(runtime.APIVersionInternal))
		if err != nil {
			return nil, err
		}
		klog.V(3).Infof("Converting from %s to %s",
			gvk.Version,
			runtime.APIVersionInternal)
		err = scheme.Convert(obj, iObj, nil)
		if err != nil {
			return nil, err
		}
		obj = iObj
	...
	// Convert to desired version (typed)
	versionedObj, err := scheme.New(targetGVK)
	if err != nil {
		return nil, err
	}
	klog.V(3).Infof("Converting from %s to %s",
		runtime.APIVersionInternal,
		targetGVK.Version)
	err = scheme.Convert(obj, versionedObj, nil)
	if err != nil {
		return nil, err
	}
