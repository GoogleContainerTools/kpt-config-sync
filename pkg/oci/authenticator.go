...
//
// Neither the credentials or token are cached by this wrapper.
// Inject the CachingCredentialProvider to add caching.
...
func (a *CredentialAuthenticator) Authorization() (*authn.AuthConfig, error) {
	...
	token, err := auth.FetchToken(context.TODO(), a.CredentialProvider)
	if err != nil {
		return nil, err
	}
