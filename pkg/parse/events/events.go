...
// GenerateFunc generates events and calls the HandleEventFunc for each one.
// Stops and returns when the Context is Done.
type GenerateFunc func(context.Context, HandleEventFunc)
...
type HandleResult struct {
	// DelayStatusUpdate tells the GenerateFunc to reset the status update timer.
...
// ResyncEvent triggers a full resync, with an empty/reset cache.
type ResyncEvent struct{}

// Type of the event
func (re ResyncEvent) Type() string {
	return "ResyncEvent"
}
...
// RetryEvent is an event that triggers a retry.
//
// See SkipChan() for how to handle skipping a retry.
// See ResetRetryBackoff() for how to handle resetting the retry backoff.
type RetryEvent struct {
	// ResetRetryBackoff should be called by the event consumer when necessary
	// to signal to the event producer that it should reset the retry backoff
	// period and start retrying again more frequently. This should generally
	// be called if there was a significant change to the inputs or source.
	//
	// TODO: Call this after successful syncs too?
	ResetRetryBackoff func()
}

// Type of the event
func (re RetryEvent) Type() string {
	return "RetryEvent"
}
