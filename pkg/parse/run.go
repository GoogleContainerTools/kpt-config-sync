...
type RunOpts struct {
	RunFunc      RunFunc
	GenerateFunc events.GenerateFunc
}
...
func Run(ctx context.Context, p Parser, nsControllerState *namespacecontroller.State, runOpts RunOpts) {
	...
	runOpts.GenerateFunc(ctx, func(event events.Event) events.HandleResult {
		switch typedEvent := event.(type) {
		case events.ResyncEvent:
			...
			runOpts.RunFunc(ctx, p, triggerResync, state)
		...
		case events.SyncEvent:
			...
			// happen until the ongoing reconciliation is done.
			runOpts.RunFunc(ctx, p, triggerReimport, state)
		...
		case events.NamespaceResyncEvent:
			...
			// The cached needToRetry will not be reset to avoid resetting the backoff retries.
			state.resetPartialCache()
			runOpts.RunFunc(ctx, p, namespaceEvent, state)

		case events.RetryEvent:
