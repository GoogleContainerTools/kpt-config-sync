...
func Run(ctx context.Context, p Parser, nsControllerState *namespacecontroller.State, runOpts RunOpts) {
	...
	events.Execute(ctx, runOpts.Schedule(ctx), func(event events.Event) events.HandleResult {
		switch event.(type) {
		case events.FullSyncEvent:
			...
			runOpts.Run(ctx, p, triggerResync, state)
		...
		case events.SyncEvent:
			...
			// happen until the ongoing reconciliation is done.
			runOpts.Run(ctx, p, triggerReimport, state)
		...
		case events.NamespaceResyncEvent:
			// If the namespace controller indicates that an update is needed,
			// attempt to re-sync.
			if !nsControllerState.ScheduleSync() {
				// Tell the event producer that we did NOT call the RunFunc.
				return events.HandleResult{DelayStatusUpdate: false}
			}
			...
			// The cached needToRetry will not be reset to avoid resetting the backoff retries.
			state.resetPartialCache()
			runOpts.Run(ctx, p, namespaceEvent, state)

		case events.RetrySyncEvent:
			...
			// being retried.
			result := runOpts.Run(ctx, p, trigger, state)

			// If the commit changed, then reset the retry backoff for the new commit.
			if result.CommitChanged {
				return events.HandleResult{
					DelayStatusUpdate: true,
		...
		// Unless otherwise instructed, assume the RunFunc was executed and
		// delay the next sync status update.
		return events.HandleResult{DelayStatusUpdate: true}
	})
}
