...
func (u *Updater) Errors() status.MultiError {
	...
	return u.prependRemediatorErrors(errs)
}
...
func dedupeConflictErrors(updaterConflictErrors, remediatorConflictErrors []status.ManagementConflictError) status.MultiError {
	conflictErrMap := orderedmap.NewOrderedMap[core.ID, status.ManagementConflictError]()
	...
	for e := conflictErrMap.Front(); e != nil; e = e.Next() {
		conflictErrs = status.Append(conflictErrs, e.Value)
	}
	return conflictErrs
}
