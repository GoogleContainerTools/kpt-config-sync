...
func (f *RepoSyncFinalizer) Finalize(ctx context.Context, syncObj client.Object) error {
	...
	deletionPropagation, ok := rs.Annotations[metadata.DeletionPropagationPolicyAnnotationKey]
	if ok && deletionPropagation == string(metadata.DeletionPropagationPolicyForeground) {
...
// updates the ReconcilerFinalizerFailure condition on the specified object.
func (f *RepoSyncFinalizer) orphanManagedObjects(ctx context.Context, syncObj *v1beta1.RepoSync) error {
	unmanageErr := f.unmanageObjects(ctx, client.ObjectKeyFromObject(syncObj))
	// Update the FinalizerFailure condition whether the destroy succeeded or failed
