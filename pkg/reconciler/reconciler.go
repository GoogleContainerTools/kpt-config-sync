...
func Run(opts Options) {
	...
	eventProducer := &events.TimedEventProducer{
	...
	if opts.ReconcilerScope == declared.RootScope {
		...
		if opts.DynamicNSSelectorEnabled {
			...
			eventProducer.NamespaceControllerPeriod = time.Second
		}
	...
	parse.Run(ctx, parser, nsControllerState, parse.RunOpts{
		RunFunc:      parse.DefaultRunFunc,
		GenerateFunc: eventProducer.Generate,
	})
