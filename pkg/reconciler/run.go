...
func Run(opts Options) {
	...
	if opts.ReconcilerScope == declared.RootScope {
		rootParseOpts := &syncclient.RootOptions{
			Options:                  parseOpts,
		...
		reconciler = NewRootSyncReconciler(reconcilerOpts, rootParseOpts)
	} else {
		reconciler = NewRepoSyncReconciler(reconcilerOpts, parseOpts)
	}
	...
	funnel := &events.Funnel{
		...
		Subscriber: NewEventHandler(ctx, reconciler, nsControllerState),
	}
