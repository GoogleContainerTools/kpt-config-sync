...
func setupOtelReconciler(t *testing.T, credentialprovider askpass.CredentialProvider, objs ...client.Object) (*syncerFake.Client, *OtelReconciler) {
	t.Helper()
...
func TestOtelReconciler(t *testing.T) {
	...
	credentialProvider := &askpass.FakeCredentialProvider{
...
func TestOtelReconcilerGooglecloud(t *testing.T) {
	...
	reqNamespacedName := namespacedName(metrics.OtelCollectorName, configmanagement.MonitoringNamespace)
	credentialProvider := &askpass.FakeCredentialProvider{
		CredentialsOut: auth.NewCredentials(&auth.CredentialsOptions{
