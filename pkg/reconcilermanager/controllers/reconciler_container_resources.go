...
// ReconcilerContainerResourceDefaults are the default resources to use when autoscaling
// (VPA) is NOT configured for the reconciler deployment.
...
func ReconcilerContainerResourceDefaults() map[string]v1beta1.ContainerResourcesSpec {
	return map[string]v1beta1.ContainerResourcesSpec{
		...
		metrics.OtelAgentName: {
			...
			CPURequest:    resource.MustParse("10m"),
			// TODO: CPU limits considered harmful.
			// A throttled otel-agent is also much more likely to drop metrics.
			CPULimit: resource.MustParse("1000m"),
			// TODO: If we want to avoid OOMKill, set Request = Limit for Guaranteed QoS.
			// Burstable pods are the most likely to be OOM killed by the kernel.
			MemoryRequest: resource.MustParse("100Mi"),
...
// ReconcilerContainerResourceAutoscaleDefaults are the default resource to use when
// autoscaling (VPA) is configured for the reconciler deployment.
// These defaults should be less than the defaults in the manifest yaml, so
// that the reconciler starts with minimal requirements and scales up as-needed.
func ReconcilerContainerResourceAutoscaleDefaults() map[string]v1beta1.ContainerResourcesSpec {
	return map[string]v1beta1.ContainerResourcesSpec{
		reconcilermanager.Reconciler: {
			ContainerName: reconcilermanager.Reconciler,
			CPURequest:    resource.MustParse("10m"),
...
func setContainerResourceDefaults(overrides []v1beta1.ContainerResourcesSpec, defaultsMap map[string]v1beta1.ContainerResourcesSpec) []v1beta1.ContainerResourcesSpec {
	...
	for _, override := range overrides {
		// TODO: validate no dupes & known container names
