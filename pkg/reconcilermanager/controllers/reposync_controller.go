...
func (r *RepoSyncReconciler) validateRepoSync(ctx context.Context, rs *v1beta1.RepoSync, reconcilerName string) error {
	...
	if err := r.validateDependencies(ctx, rs, reconcilerName); err != nil {
		return err
	}
...
func (r *RepoSyncReconciler) validateNamespaceSecret(ctx context.Context, repoSync *v1beta1.RepoSync, reconcilerName string) status.Error {
	...
	if errs := validation.IsDNS1123Subdomain(secretName); errs != nil {
		return validate.InvalidSecretName(errs, secretName)
	}
	...
	if err != nil {
		...
		return validate.ErrorFetchingSecret(namespaceSecretName, err)
	}

	_, r.knownHostExist = secret.Data[KnownHostsKey]
	r.githubApp = githubAppFromSecret(secret)

	if err := validateSecretData(authType, secret); err != nil {
