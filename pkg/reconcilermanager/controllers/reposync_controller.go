...
func NewRepoSyncReconciler(clusterName string, reconcilerPollingPeriod, hydrationPollingPeriod time.Duration, cache cache.Cache, client client.Client, watcher client.WithWatch, dynamicClient dynamic.Interface, log logr.Logger, scheme *runtime.Scheme) *RepoSyncReconciler {
...
func (r *RepoSyncReconciler) SetupWithManager(ctx context.Context, mgr controllerruntime.Manager, watchFleetMembership bool) error {
	// Index the `gitSecretRefName` field, so that we will be able to lookup RepoSync be a referenced `SecretRef` name.
	if err := mgr.GetFieldIndexer().IndexField(ctx, &v1beta1.RepoSync{}, gitSecretRefField, func(rawObj client.Object) []string {
	...
	if err := mgr.GetFieldIndexer().IndexField(ctx, &v1beta1.RepoSync{}, caCertSecretRefField, func(rawObj client.Object) []string {
		rs, ok := rawObj.(*v1beta1.RepoSync)
		if !ok {
			// Only add index for RepoSync
			return nil
	...
	if err := mgr.GetFieldIndexer().IndexField(ctx, &v1beta1.RepoSync{}, helmSecretRefField, func(rawObj client.Object) []string {
		rs, ok := rawObj.(*v1beta1.RepoSync)
		if !ok {
			// Only add index for RepoSync
			return nil
