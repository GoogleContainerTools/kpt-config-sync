...
func TestRepoSyncReconcileWithInvalidCACertSecret(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			wantRs := k8sobjects.RepoSyncObjectV1Beta1(reposyncNs, reposyncName)
			reposync.SetStalled(wantRs, "Validation", validate.MissingKeyInSecret(fmt.Sprintf("caCertSecretRef was set, but %s key is not present in %s Secret", CACertSecretKey, caCertSecret)))
			validateRepoSyncStatus(t, wantRs, fakeClient)
		})
	}
}
...
func TestRepoSyncValidateCACertSecret(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"caCertSecretRef set but invalid Secret": {
			...
			err: validate.MissingKeyInSecret(fmt.Sprintf("caCertSecretRef was set, but %s key is not present in %s Secret", CACertSecretKey, caCertSecret)).Error(),
		},
	}
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
...
For more information, see https://g.co/cloud/acm-errors#knv1061`))
	...
	wantRs = k8sobjects.RepoSyncObjectV1Beta1(rs.Namespace, rs.Name)
	reposync.SetStalled(wantRs, "Validation",
		validate.MissingKeyInSecret(fmt.Sprintf(`git secretType was set as "githubapp" but github-app-private-key key is not present in %s secret`, secretRef)))
	validateRepoSyncStatus(t, wantRs, fakeClient)
