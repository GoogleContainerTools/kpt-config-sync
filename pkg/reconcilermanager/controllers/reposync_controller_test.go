...
func TestRepoSyncReconcileWithInvalidCACertSecret(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			wantRs := k8sobjects.RepoSyncObjectV1Beta1(reposyncNs, reposyncName)
			reposync.SetStalled(wantRs, "Validation", validate.InvalidSource(tc.repoSync.Kind, fmt.Errorf("caCertSecretRef was set, but %s key is not present in %s Secret", CACertSecretKey, caCertSecret)))
			validateRepoSyncStatus(t, wantRs, fakeClient)
		})
	}
}
