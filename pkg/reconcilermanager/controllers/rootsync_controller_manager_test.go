	...
	err = watchObjectUntil(ctx, fakeClient.Scheme(), watcher, core.ObjectNamespacedName(rs), func(event watch.Event) error {
		...
		if event.Type == watch.Modified {
			...
			for _, cond := range rsObj.Status.Conditions {
				if cond.Reason == "Validation" && cond.Message == validate.MissingKeyInSecret(`git secretType was set as "token" but token key is not present in root-ssh-key secret`).Error() {
					return nil
				}
			}
