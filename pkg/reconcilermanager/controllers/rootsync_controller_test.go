...
func TestRootSyncReconcileWithInvalidCACertSecret(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			rootsync.SetStalled(wantRs, "Validation", validate.MissingCertKey(CACertSecretKey, caCertSecret))
...
func TestRootSyncValidateCACertSecret(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"caCertSecretRef set but invalid Secret": {
			...
			err: validate.MissingCertKey(CACertSecretKey, caCertSecret).Error(),
		},
	}
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
...
For more information, see https://g.co/cloud/acm-errors#knv1061`))
	...
	wantRs = k8sobjects.RootSyncObjectV1Beta1(rs.Name)
	rootsync.SetStalled(wantRs, "Validation",
		validate.InvalidSecret(fmt.Errorf(`git secretType was set as "githubapp" but github-app-private-key key is not present in %s secret`, secretRef)))
	validateRootSyncStatus(t, wantRs, fakeClient)
