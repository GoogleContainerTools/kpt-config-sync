...
func TestValidateSecretData(t *testing.T) {
	testCases := map[string]struct {
		...
		"Invalid Github App auth with missing private key": {
			...
			wantError: validate.MissingKeyInSecret(`git secretType was set as "githubapp" but github-app-private-key key is not present in foo secret`),
		},
		"Invalid Github App auth with missing installation ID": {
			...
			wantError: validate.MissingKeyInSecret(`git secretType was set as "githubapp" but github-app-installation-id key is not present in foo secret`),
		},
		"Invalid Github App auth with client ID AND application ID": {
			...
			wantError: validate.InvalidIDInSecret(`git secretType was set to "githubapp" but more than one of (github-app-application-id, github-app-client-id) is present in foo secret`),
		},
		"Invalid Github App auth with neither client ID NOR application ID": {
			...
			wantError: validate.InvalidIDInSecret(`git secretType was set to "githubapp" but one of (github-app-application-id, github-app-client-id) is not present in foo secret`),
		},
		...
		"Usupported auth": {
			auth:      "( ͡° ͜ʖ ͡°)",
			wantError: validate.InvalidSecretType(`( ͡° ͜ʖ ͡°)`),
		},
	}
