...
func RepoSyncSpec(sourceType string, git *v1beta1.Git, oci *v1beta1.Oci, helm *v1beta1.HelmRepoSync, rs client.Object) status.Error {
	...
	case v1beta1.GitSource:
		return GitSpec(git, rs, syncKind)
	case v1beta1.OciSource:
		return OciSpec(oci, rs, syncKind)
	case v1beta1.HelmSource:
		return HelmSpec(reposync.GetHelmBase(helm), rs, syncKind)
	default:
		return InvalidSourceType(rs, syncKind)
	}
}
...
func RootSyncSpec(sourceType string, git *v1beta1.Git, oci *v1beta1.Oci, helm *v1beta1.HelmRootSync, rs client.Object) status.Error {
	...
	case v1beta1.OciSource:
		return OciSpec(oci, rs, syncKind)
	case v1beta1.HelmSource:
		if err := HelmSpec(rootsync.GetHelmBase(helm), rs, syncKind); err != nil {
			return err
		}
		if helm.Namespace != "" && helm.DeployNamespace != "" {
			return HelmNSAndDeployNS(rs, syncKind)
		}
		return nil
	default:
		return InvalidSourceType(rs, syncKind)
...
func GitSpec(git *v1beta1.Git, rs client.Object, syncKind string) status.Error {
	if git == nil {
		return MissingGitSpec(rs, syncKind)
	}

	// We can't connect to the git repo if we don't have the URL.
	if git.Repo == "" {
		return MissingGitRepo(rs, syncKind)
	}
	...
	case configsync.AuthGCPServiceAccount:
		if git.GCPServiceAccountEmail == "" {
			return MissingGCPSAEmail(rs, syncKind)
		}
		if !validGCPServiceAccountEmail(git.GCPServiceAccountEmail) {
			return InvalidGCPSAEmail(rs, syncKind)
		}
	default:
		return InvalidAuthType(rs, syncKind)
	}
	...
	case configsync.AuthNone, configsync.AuthGCENode, configsync.AuthGCPServiceAccount:
		if git.SecretRef != nil && git.SecretRef.Name != "" {
			return IllegalSecretRef(rs, syncKind)
		}
	default:
		if git.SecretRef == nil || git.SecretRef.Name == "" {
			return MissingSecretRef(rs, syncKind)
		}
	}
...
func OciSpec(oci *v1beta1.Oci, rs client.Object, syncKind string) status.Error {
	if oci == nil {
		return MissingOciSpec(rs, syncKind)
	}

	// We can't connect to the oci image if we don't have the URL.
	if oci.Image == "" {
		return MissingOciImage(rs, syncKind)
	}
	...
	default:
		return InvalidOciAuthType(rs, syncKind)
	}
	return nil
}

// HelmSpec validates the Helm specification for any obvious problems.
func HelmSpec(helm *v1beta1.HelmBase, rs client.Object, syncKind string) status.Error {
	if helm == nil {
		return MissingHelmSpec(rs, syncKind)
	}

	// We can't locate the helm chart if we don't have the URL.
	if helm.Repo == "" {
		return MissingHelmRepo(rs, syncKind)
	}

	// We can't locate the helm chart if we don't have the chart name.
	if helm.Chart == "" {
		return MissingHelmChart(rs, syncKind)
	}
	...
	default:
		return InvalidHelmAuthType(rs, syncKind)
	}

	for _, vf := range helm.ValuesFileRefs {
		if vf.Name == "" {
			return MissingHelmValuesFileRefsName(rs, syncKind)
		}
	}
...
func ValuesFileRefs(ctx context.Context, cl client.Client, rs client.Object, syncKind string, valuesFileRefs []v1beta1.ValuesFileRef) status.Error {
	for _, vf := range valuesFileRefs {
		...
		if err := cl.Get(ctx, objRef, &cm); err != nil {
			return HelmValuesMissingConfigMap(rs, syncKind, err)
		}
		if cm.Immutable == nil || !(*cm.Immutable) {
			return HelmValuesConfigMapMustBeImmutable(rs, syncKind, objRef.Name)
		}
		dataKey := HelmValuesFileDataKeyOrDefault(vf.DataKey)
		if _, found := cm.Data[dataKey]; !found {
			return HelmValuesMissingConfigMapKey(rs, syncKind, objRef.Name, dataKey)
		}
	}
	return nil
}
...
func MissingGitSpec(o client.Object, syncKind string) status.Error {
	return invalidSyncBuilder.
		Sprintf("%ss must specify spec.git when spec.sourceType is %q",
			syncKind, v1beta1.GitSource).
...
func MissingGitRepo(o client.Object, syncKind string) status.Error {
	return invalidSyncBuilder.
		Sprintf("%ss must specify spec.git.repo when spec.sourceType is %q",
			syncKind, v1beta1.GitSource).
		BuildWithResources(o)
}
...
// methods.
func InvalidAuthType(o client.Object, syncKind string) status.Error {
...
// do nothing.
func NoOpProxy(o client.Object, syncKind string) status.Error {
	return invalidSyncBuilder.
...
func InvalidGCPSAEmail(o client.Object, syncKind string) status.Error {
	return invalidSyncBuilder.
		Sprintf("%ss which specify spec.git.auth or spec.oci.auth as %q must use suffix <gcp_serviceaccount_name>.[%s]",
			syncKind, configsync.AuthGCPServiceAccount, gcpSASuffix).
		BuildWithResources(o)
}
...
func MissingGCPSAEmail(o client.Object, syncKind string) status.Error {
	return invalidSyncBuilder.
		Sprintf("%ss which specify spec.git.auth or spec.oci.auth as %q must also specify spec.git.gcpServiceAccountEmail or spec.oci.gcpServiceAccountEmail",
			syncKind, configsync.AuthGCPServiceAccount).
		BuildWithResources(o)
}
...
func InvalidSourceType(o client.Object, syncKind string) status.Error {
	return invalidSyncBuilder.
		Sprintf("%ss must specify spec.sourceType to be one of %q, %q, %q",
			syncKind, v1beta1.GitSource, v1beta1.OciSource, v1beta1.HelmSource).
		BuildWithResources(o)
}

// MissingOciSpec reports that a RootSync/RepoSync doesn't declare the OCI spec
// when spec.sourceType is set to `oci`.
func MissingOciSpec(o client.Object, syncKind string) status.Error {
	return invalidSyncBuilder.
		Sprintf("%ss must specify spec.oci when spec.sourceType is %q",
			syncKind, v1beta1.OciSource).
		BuildWithResources(o)
}

// MissingOciImage reports that a RootSync/RepoSync doesn't declare the OCI image it is
// supposed to connect to.
func MissingOciImage(o client.Object, syncKind string) status.Error {
	return invalidSyncBuilder.
		Sprintf("%ss must specify spec.oci.image when spec.sourceType is %q",
			syncKind, v1beta1.OciSource).
		BuildWithResources(o)
}
...
func InvalidOciAuthType(o client.Object, syncKind string) status.Error {
...
func MissingHelmSpec(o client.Object, syncKind string) status.Error {
	return invalidSyncBuilder.
		Sprintf("%ss must specify spec.helm when spec.sourceType is %q",
			syncKind, v1beta1.HelmSource).
		BuildWithResources(o)
}
...
func MissingHelmRepo(o client.Object, syncKind string) status.Error {
	return invalidSyncBuilder.
		Sprintf("%ss must specify spec.helm.repo when spec.sourceType is %q",
			syncKind, v1beta1.HelmSource).
		BuildWithResources(o)
}
...
func MissingHelmChart(o client.Object, syncKind string) status.Error {
	return invalidSyncBuilder.
		Sprintf("%ss must specify spec.helm.chart when spec.sourceType is %q",
			syncKind, v1beta1.HelmSource).
		BuildWithResources(o)
}
...
// methods for Helm.
func InvalidHelmAuthType(o client.Object, syncKind string) status.Error {
...
func HelmNSAndDeployNS(o client.Object, syncKind string) status.Error {
	return invalidSyncBuilder.
...
func MissingHelmValuesFileRefsName(o client.Object, syncKind string) status.Error {
	return invalidSyncBuilder.
...
func HelmValuesMissingConfigMap(o client.Object, syncKind string, err error) status.Error {
	return invalidSyncBuilder.
...
func HelmValuesMissingConfigMapKey(o client.Object, syncKind string, name, key string) status.Error {
	return invalidSyncBuilder.
		Sprintf("%ss must reference valid ConfigMaps in spec.helm.valuesFileRefs: ConfigMap %q in namespace %q does not have data key %q",
			syncKind, name, o.GetNamespace(), key).
		BuildWithResources(o)
}

// HelmValuesConfigMapMustBeImmutable reports that a referenced ConfigMap from RSync spec.helm.valuesFileRefs is
// not immutable.
func HelmValuesConfigMapMustBeImmutable(o client.Object, syncKind string, name string) status.Error {
	return invalidSyncBuilder.
		Sprintf("%ss must reference valid ConfigMaps in spec.helm.valuesFileRefs: ConfigMap %q in namespace %q is not immutable",
			syncKind, name, o.GetNamespace()).
		BuildWithResources(o)
}
