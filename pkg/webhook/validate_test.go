...
func TestValidator_Handle(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			user: configSyncNamespaceReconciler("bookstore", repoSyncName),
			deny: `config sync "ns-reconciler-bookstore-my-repo-sync-12" can not DELETE object "Role.rbac.authorization.k8s.io, world/hello" managed by config sync "ns-reconciler-videostore-my-repo-sync-12"`,
		},
		{
			name: "Namespace reconciler deletes an object it does not manage",
			oldObj: fake.RoleObject(
		...
		},
		{
			...
			user: bob(),
			deny: `bob@acme.com is not authorized to create managed resource "rbac.authorization.k8s.io_role_world_hello"`,
		},
		{
			...
			user: bob(),
			deny: `bob@acme.com is not authorized to delete managed resource "rbac.authorization.k8s.io_role_world_hello"`,
		},
		{
		...
		},
		{
			...
			user: bob(),
			deny: `bob@acme.com cannot modify fields of object "rbac.authorization.k8s.io_role_world_hello" managed by Config Sync: .rules`,
		},
		{
			...
			user: bob(),
			deny: `bob@acme.com cannot modify Config Sync metadata of object "rbac.authorization.k8s.io_role_world_hello": .labels.app.kubernetes.io/managed-by`,
		},
		{
		...
		},
		{
			...
			user: bob(),
			deny: `bob@acme.com is not authorized to modify ResourceGroup.kpt.dev generated by Config Sync`,
		},
		{
		...
		},
		{
			...
			newObj: fake.RoleObject(
				...
				core.Annotation(csmetadata.ResourceIDKey, "rbac.authorization.k8s.io_role_default-name"),
				core.Annotation(csmetadata.LifecycleMutationAnnotation, "other")),
			deny: ` cannot modify Config Sync metadata of object "rbac.authorization.k8s.io_role_default-name": .annotations.client.lifecycle.config.k8s.io/mutation`,
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			} else if tc.deny != resp.Result.Message {
				t.Errorf("got Handle() response denied %q, want denied %q", resp.Result.Message, tc.deny)
			}
		})
	}
}
